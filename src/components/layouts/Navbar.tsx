      ...
      <nav className="cockpit-panel mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 rounded-2xl px-3 py-3 text-[11px] uppercase tracking-[0.16em] md:px-5">
        ...
        <div className="hidden items-center gap-2 lg:flex">
          {links.map((item) => {
            ...
            return (
              <Link key={item.to} to={item.to} data-magnetic className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 transition ${isActive ? 'border-[--color-primary]/60 bg-[--color-primary]/20 text-[--color-primary] shadow-[0_0_18px_color-mix(in_oklab,var(--color-primary)_45%,transparent)]' : 'border-[color:color-mix(in_oklab,var(--text-color)_14%,transparent)] text-foreground/70 hover:border-[--color-primary]/40 hover:text-[--color-primary]'}`}>
        ...
        <div className="flex items-center gap-3 text-[10px] text-foreground/75">
          <div className="hud-surface hidden items-center gap-3 rounded-xl px-3 py-1.5 md:flex">
          ...
          <button type="button" onClick={() => { setSoundOn((v) => !v); playBeep() }} className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-[color:color-mix(in_oklab,var(--text-color)_14%,transparent)] bg-[var(--hud-surface)] text-foreground/80">
            {soundOn ? <Volume2 className="h-4 w-4" /> : <VolumeX className="h-4 w-4" />}
          </button>
          <button type="button" onClick={() => { toggleTheme(); playBeep() }} className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-[color:color-mix(in_oklab,var(--text-color)_14%,transparent)] bg-[var(--hud-surface)] text-foreground/80">
        ...
        {menuOpen && (
          <div className="hud-surface w-full rounded-xl p-3 lg:hidden">
              ...
              {links.map((item) => {
                ...
                return (
                  <Link
                    ...
                    className={`inline-flex items-center gap-2 rounded-full border px-3 py-2 transition ${isActive ? 'border-[--color-primary]/60 bg-[--color-primary]/20 text-[--color-primary]' : 'border-[color:color-mix(in_oklab,var(--text-color)_14%,transparent)] text-foreground/80 hover:border-[--color-primary]/40 hover:text-[--color-primary]'}`}
                  >
                    <Icon className="h-3.5 w-3.5" />
                    {item.label}
                  </Link>
